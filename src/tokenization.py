...
class P5Tokenizer(T5Tokenizer):
    def __init__(
        ...
        **kwargs
        ...
        # Call the parent (T5Tokenizer) initialiser so all expected attributes
        # such as ``legacy`` and ``sp_model`` are populated.
        super().__init__(
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *init_inputs, **kwargs):
        """Ensure we always build a ``P5Tokenizer`` even if the checkpoint's
        ``tokenizer_config.json`` points to ``T5Tokenizer``.

        The HuggingFace loader inspects ``tokenizer_class`` when deciding which
        class to instantiate.  Our checkpoints ship with the default
        ``"tokenizer_class": "T5Tokenizer"`` so a plain ``super().from_pretrained``
        would hand back a vanilla ``T5Tokenizer`` instance, missing the
        ``sp_model`` and extra token machinery we rely on.

        To stay compatible we first load a base ``T5Tokenizer`` to get the
        SentencePiece model path, then manually construct ``P5Tokenizer`` using
        the same vocabulary file and special-token configuration.
        """

        extra_ids = kwargs.pop("extra_ids", 100)
        user_extra_ids = kwargs.pop("user_extra_ids", 0)
        item_extra_ids = kwargs.pop("item_extra_ids", 0)

        # Load the underlying tokenizer to locate the vocab / spiece file.
        ...
        tokenizer = cls(
            vocab_file=base_tokenizer.vocab_file,
            ...
            additional_special_tokens=additional_special_tokens,
            legacy=getattr(base_tokenizer, "legacy", True),
        )

        # Mirror relevant settings from the base tokenizer so downstream code
        # sees consistent behaviour (e.g. max length, added vocab cache ...).
        tokenizer.model_max_length = base_tokenizer.model_max_length
        tokenizer.init_kwargs = getattr(base_tokenizer, "init_kwargs", {})
